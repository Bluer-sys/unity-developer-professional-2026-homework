...
namespace Game
{
	public static class SpatialHashUseCase
	{
		...
		{
			return Hash(GetCell(position, cellSize));
		}

		public static uint Hash(int2 cell)
		{
			return math.hash(cell);
		}
			...
			where TPredicate : struct, IEntityPredicate
		{
			...
			var cellRadius = (int)math.ceil(radius / spatialHash.cellSize);
			...
			for (int x = -cellRadius; x <= cellRadius; x++)
				for (int z = -cellRadius; z <= cellRadius; z++)
				{
					...
					if (!spatialHash.map->TryGetFirstValue(hash,
