...
namespace Game
{
	public struct IsEnemyPredicate : IEntityPredicate
	{
		...
		{
			return entity != _self && _teamLookup.TryGetComponent(entity, out Team team) && team.value != _team && _healthLookup.TryGetComponent(entity, out Health health) && health.IsAlive();
		}
	}
}
