...
namespace SnivelerCode.GpuAnimation.Editor.Window
{
    ...
    public sealed class GeneratorWindow : EditorWindow
    {
        ...
        private const string _bonesUxml = _guiPath + "BonesTemplate.uxml";
        ...
        public void CreateGUI()
        {
            ...
            useDqs.value = _instance.UseDqs;
            useDqs.RegisterValueChangedCallback(evt =>
                _instance.UseDqs = evt.newValue);

            var rootMotion = RootQuery<Toggle>("RootMotion");
            rootMotion.value = _instance.RootMotion;
            rootMotion.RegisterValueChangedCallback(evt =>
                _instance.RootMotion = evt.newValue);
        ...
        private void PrefabChange(ChangeEvent<Object> evt)
        {
            ...
            var bonesContent = RootQuery<VisualElement>("BonesContent");
            bonesContent.Clear();
            RootQuery<Button>("AddBone").RegisterCallback<ClickEvent>(_ =>
            {
                _instance.AddBone();
                RenderBoneElement(bonesContent);
            });
        }
        ...
        {
            if (shader.FindPropertyIndex(AnimatorStrings.InstanceID) == -1) return false;
            if (!shader.keywordSpace.FindKeyword(AnimatorShaderKeywords.Dqs).isValid) return false;
            return true;
        }

        private void RenderBoneElement(VisualElement content)
        {
            var template = AssetDatabase
                .LoadAssetAtPath<VisualTreeAsset>(_bonesUxml)
                .Instantiate();

            content.Add(template);

            template.Q<Label>("BoneTitle").text = "Bake Bone";

            int index = content.IndexOf(template);
            var dropdown = template.Q<DropdownField>("BoneList");
            dropdown.choices = _instance.MasterBones.Select(bone => bone.name).ToList();
            dropdown.value = _instance.Bones[index];
            dropdown.RegisterCallback<ChangeEvent<string>>(evt =>
            {
                int changeIndex = content.IndexOf(template);
                _instance.Bones[changeIndex] = evt.newValue;
            });

            template.Q<Button>("Remove").RegisterCallback<ClickEvent>(_ =>
            {
                int removeIndex = content.IndexOf(template);
                _instance.Bones.RemoveAt(removeIndex);
        ...
        private void RenderLodElement(LodInstance lodInstance, int lodIndex, VisualElement content)
        {
            ...
            dropdown.RegisterValueChangedCallback(evt =>
                lodInstance.SkinningQuality = (LodInstance.Quality) Array.IndexOf(qualityNames, evt.newValue));

            var interpolationField = lodTemplate.Q<Toggle>("Interpolation");
            interpolationField.value = lodInstance.Interpolation;
            interpolationField.RegisterValueChangedCallback(evt =>
                lodInstance.Interpolation = evt.newValue);

            var shadowsField = lodTemplate.Q<Toggle>("Shadows");
            shadowsField.value = lodInstance.Shadows;
            shadowsField.RegisterValueChangedCallback(evt =>
                lodInstance.Shadows = evt.newValue);
