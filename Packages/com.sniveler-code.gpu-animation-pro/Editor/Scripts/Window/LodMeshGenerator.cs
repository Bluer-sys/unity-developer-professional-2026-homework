            ...
            for (int i = 0; i < instance.Lods.Count; ++i)
            {
                ...
                for (int k = 0; k < finalMergedMesh.subMeshCount; ++k)
                {
                    ...
                    if (!materialSet.ContainsKey(materialGuid))
                    {
                        ...
                        if (instance.UseDqs) material.EnableKeyword(AnimatorShaderKeywords.Dqs);
                        else material.DisableKeyword(AnimatorShaderKeywords.Dqs);

                        if (lodInstance.Interpolation) material.DisableKeyword(AnimatorShaderKeywords.AnimStepOnly);
                        else material.EnableKeyword(AnimatorShaderKeywords.AnimStepOnly);

                        switch (lodInstance.SkinningQuality)
                        {
                            case LodInstance.Quality.Bones4:
                                material.DisableKeyword(AnimatorShaderKeywords.BoneLod01);
                                material.DisableKeyword(AnimatorShaderKeywords.BoneLod02);
                                break;

                            case LodInstance.Quality.Bones2:
                                material.DisableKeyword(AnimatorShaderKeywords.BoneLod01);
                                material.EnableKeyword(AnimatorShaderKeywords.BoneLod02);
                                break;

                            case LodInstance.Quality.Bones1:
                                material.EnableKeyword(AnimatorShaderKeywords.BoneLod01);
                                material.DisableKeyword(AnimatorShaderKeywords.BoneLod02);
                                break;
                        }
                ...
                meshRenderer.shadowCastingMode = lodInstance.Shadows ? ShadowCastingMode.On : ShadowCastingMode.Off;
