...
using System.Linq;
using SnivelerCode.GpuAnimation.Editor.Components;
...
namespace SnivelerCode.GpuAnimation.Editor.Window
{
    public sealed class AnimationBaker : IAnimationBaker
    {
        public AnimationBakeResult BakeAnimations(PrefabInstance instance)
        {
            ...
            {
                BakedMatricesDqs = new List<DualQuaternion>(),
                ...
                MonoBones = new MonoBlobBones(instance.MasterBones),
            ...
            for (int boneIndex = 0; boneIndex < instance.MasterBones.Length; boneIndex++)
            {
                ...
                if (instance.UseDqs) result.BakedMatricesDqs.Add(new DualQuaternion(finalMatrix));
                else result.BakedMatricesLbs.Add(finalMatrix.Compress());

                if (instance.Bones.Contains(bone.name))
                {
                    result.MonoBones.Add(boneIndex, 0, localBoneMatrix);
                }
            }
            ...
            foreach (ClipInstance clip in enabledAnimations)
            {
                ...
                var materialAnimation = new MonoBlobAnimator
                {
                    FPS = (byte) clip.Fps,
                    Frames = frameCount,
                    Start = instance.UseDqs
                        ? (uint) result.BakedMatricesDqs.Count
                        : (uint) result.BakedMatricesLbs.Count,
                ...
                animatorComponent.applyRootMotion = instance.RootMotion;
                ...
                for (int frame = 0; frame < frameCount; frame++)
                {
                    ...
                    Vector3 rootPos = clonedPrefab.transform.position;
                    Quaternion rootRot = clonedPrefab.transform.rotation;

                    Matrix4x4 rootMatrix = Matrix4x4.identity;
                    if (instance.RootMotion)
                    {
                        rootMatrix = Matrix4x4.TRS(rootPos, rootRot, Vector3.one);
                        materialAnimation.RootMotionFrames.Add(new RigidTransform(rootMatrix));
                    }
                    ...
                    for (int boneIndex = 0; boneIndex < clonedMasterBones.Length; boneIndex++)
                    {
                        ...
                        if (instance.UseDqs) result.BakedMatricesDqs.Add(new DualQuaternion(matrix));
                        else result.BakedMatricesLbs.Add(matrix.Compress());

                        // sockets
                        if (instance.Bones.Contains(bone.name))
                        {
                            Matrix4x4 fullWorldBone = localBoneMatrix * scaleMatrix;
                            result.MonoBones.Add(boneIndex, result.Animations.Count, fullWorldBone);
                        }
                    }
            ...
            for (int j = 0; j < enabledAnimations.Count; ++j)
            {
                ...
                foreach (AnimatorStateTransition transition in animatorState.state.transitions)
                {
                    ...
                    if (transition.hasExitTime)
                    {
                        ...
                        float normalizedExit;

                        if (exit > 0f && math.fmod(exit, 1.0f) == 0f)
                        {
                            normalizedExit = 1.0f;
                        }
                        else
                        {
                            normalizedExit = math.fmod(exit, 1.0f);
                        }

                        float targetFrameFloat = normalizedExit * frameCount;
                        startFrame = (ushort)math.clamp(targetFrameFloat, 0, frameCount - 1);
                    }
